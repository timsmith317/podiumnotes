...
export const SUPERTONIC_MODEL_DIR = FileSystem.documentDirectory + 'onnx';
export const SUPERTONIC_STYLE_DIR = FileSystem.documentDirectory + 'voice_styles';

export function supertonicStylePath(styleId) {
  return `${SUPERTONIC_STYLE_DIR}/${styleId || 'M1'}.json`;
}

// Cheap probe: the engine is usable only if the model files are actually
// present. Checks the largest model and the config, which is enough to catch
// a partial or interrupted copy.
// iOS lists an app's folder under "On My iPhone/iPad" in the Files app only
// once its Documents directory contains something. On a fresh install ours
// is empty — and the thing that would fill it is the very model you're
// trying to copy in. Creating the audio cache directory up front breaks that
// circle, and it's somewhere the app needs anyway.
export async function ensureDocumentsVisible() {
  try {
...
export async function supertonicAvailable(styleId = VOICE_STYLE_ID) {
  await ensureDocumentsVisible();
  try {
    // TEMPORARY (remove once the Mac/iPad containers are sorted): the app's
    // Documents path differs per platform and per container, and guessing it
    // from the outside is unreliable. This prints the path the app actually
    // reads from, which is the only one that matters.
    if (__DEV__) {
      console.log('[listen] documentDirectory:', FileSystem.documentDirectory);
      console.log('[listen] looking for model at:', SUPERTONIC_MODEL_DIR);
    }
    const required = [
      `${SUPERTONIC_MODEL_DIR}/tts.json`,
      `${SUPERTONIC_MODEL_DIR}/vector_estimator.onnx`,
      `${SUPERTONIC_MODEL_DIR}/vocoder.onnx`,
      supertonicStylePath(styleId),
    ];
...
function supertonicJob(note, { wpm, steps = 8 }) {
...
export async function beginListening(note, opts) {
  ...
  return await SpeechPlayer.beginProgressive(
    job.speechText, job.dir, job.base, note.title || 'Podium Notes',
    {
      ...
      startAfterSeconds: opts?.startAfterSeconds ?? 6,
    }
  );
}
...
export async function deleteSegmentsForNote(noteId) {
  try {
