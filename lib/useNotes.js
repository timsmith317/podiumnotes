...
export function useNotes() {
  ...
  // Persist a user-defined list order. Local only — order is not synced (by
  // design; reordering is quick to redo on another device if desired). The
  // _notes array order IS the display order, so we just replace it.
  const reorderNotes = useCallback((orderedNotes) => {
    _notes = orderedNotes;
    persist();
    notify();
  }, []);

  return { notes, createNote, updateNote, deleteNote, getNote, refresh, flushSync, reorderNotes };
}
