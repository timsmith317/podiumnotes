...
export function useListen(note, wpm, active = true) {
  ...
  // composition partway into the note.
  ...
  function seek(target, resumeAfter) {
    ...
    const to = renderedRef.current || 0;
  ...
  async function restartAt(target, resume) {
    ...
    try {
      ...
      renderedFromRef.current = began.renderedFrom ?? target;
  ...
  useEffect(() => {
    ...
    subsRef.current = [
      SpeechPlayer.addProgressListener((evt) => {
        ...
        if (typeof rf === 'number') renderedFromRef.current = rf;
    ...
    try {
      if (loadedForRef.current !== key) {
        ...
        if (typeof startAt === 'number' && startAt >= 0) {
          // Where the reader is looking wins over where audio stopped.
          start = d > 0 ? Math.min(startAt, Math.max(0, d - RESUME_TAIL_SEC)) : startAt;
        } else if (d > 0 && saved >= RESUME_MIN_SEC && saved < d - RESUME_TAIL_SEC) {
          start = saved;
        }
        lastSaveRef.current = start;
        if (start > 0) {
          const r = seek(start, true);
          // A restart renders from the saved position and starts playback
          // itself. Falling through would call play() on a player that is
          // being replaced, and set 'playing' before restartAt sets its own
          // phase — which is what made the button flash play then pause.
