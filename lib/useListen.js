...
import {
  ...
  supertonicAvailable, SUPERTONIC_MODEL_DIR, beginListening,
...
export function useListen(note, wpm, active = true) {
  ...
  useEffect(() => {
    ...
    subsRef.current = [
      ...
      SpeechPlayer.addSynthProgressListener((e) => {
        ...
        const rtf = pick('rtf');
        if (__DEV__ && typeof rtf === 'number' && rtf > 0) {
          console.log(`[listen] segment rtf ${rtf.toFixed(2)}x  buffered ${(r || 0).toFixed(1)}s`);
        }
  ...
  useEffect(() => {
    ...
    supertonicAvailable().then((ready) => {
      ...
      if (ready) SpeechPlayer.prepareSynthEngine(SUPERTONIC_MODEL_DIR).catch(() => {});
    }).catch(() => {});
  }, [active]);
  ...
  async function prepareAndPlay(attempt = 0) {
    ...
    const key = `${note.id}|supertonic|${wpm}`;
    try {
      if (loadedForRef.current !== key) {
        ...
        if (__DEV__) {
          console.log('[listen] load: est', (d || 0).toFixed(1) + 's, buffered',
            renderedRef.current.toFixed(1) + 's | saved', saved.toFixed(1) +
            's → start', start.toFixed(1) + 's');
        }
