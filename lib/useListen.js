...
import {
  ...
  supertonicAvailable, modelPaths, beginListening,
  ...
  useEffect(() => {
    ...
    subsRef.current = [
      ...
      SpeechPlayer.addErrorListener((message) => {
        // Speech RECOGNITION failures are not playback failures. A disabled
        // Dictation service surfaced here as a second, bare alert stacked
        // behind voice follow's own — which already names the setting and
        // tells you how to fix it. Two dialogs for one problem, the second
        // appearing after the user had already resolved it.
