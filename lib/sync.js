...
// Diagnostic logging. Follows the build mode: every dev session traces sync
// behavior (push/pull/reconcile/delete outcomes) in Metro automatically;
// release builds stay silent. If this had been on during development, the
// ignoredWords schema-deploy failure would have been visible on day one.
const SYNC_DEBUG = __DEV__;
const PUSH_DEBOUNCE_MS = 4000;                       // backstop: collapse editing bursts into few pushes (primary push is on editor blur)
...
function log(...args) { if (SYNC_DEBUG) console.log(LOG, ...args); }
...
function noteToFields(note) {
  return {
    ...
    body:      note.body ?? '',
    kind:      note.kind ?? 'text',
    ignoredWords: JSON.stringify(note.ignoredWords ?? []),
...
function recordToNote(rec) {
  ...
  return {
    ...
    body:      f.body ?? '',
    kind:      f.kind ?? 'text',
    ignoredWords: (() => { try { return JSON.parse(f.ignoredWords ?? '[]'); } catch (e) { return []; } })(),
...
// The actual cloud write, shared by the debounced syncNote and the immediate
// flushNote. Idempotent upsert keyed by note.id.
async function pushNow(note) {
...
function pushable(note) {
  if (!_started || !_available) return false;      // local-only mode
  if (note.kind === 'pdf') return false;           // v1 scope: text only
  if (isTombstoned(note.id)) return false;
  return true;
}

export function syncNote(note) {
  if (!pushable(note)) return;
  _pendingIds.add(note.id);
  _pendingCount = _pendingIds.size;
  if (_pushTimers[note.id]) clearTimeout(_pushTimers[note.id]);
  _pushTimers[note.id] = setTimeout(() => {
    delete _pushTimers[note.id];
    pushNow(note);
  }, PUSH_DEBOUNCE_MS);
}

// Immediate push, bypassing the debounce. Called on editor blur/close so a note
// syncs once, promptly, when you're done editing — instead of repeatedly mid-
// edit. Cancels any pending debounced push for the same note first.
export function flushNote(note) {
  if (!pushable(note)) return;
  if (_pushTimers[note.id]) { clearTimeout(_pushTimers[note.id]); delete _pushTimers[note.id]; }
  return pushNow(note);
}
